    ...
    def run_detection(self):
        ...
        try:
            while True:
                ...
                self.human_detected, self.human_bbox = self.is_person_found(preds)
                ...
                if self.human_detected:
                    x1, y1, x2, y2 = map(int, self.human_bbox)
                    cv2.rectangle(full_image, (x1, y1), (x2, y2), (255, 255, 0), 2)
        ...
        for detection in det:
            for entry in detection:
                if int(entry[5]) == person_index:
                    ...
                    return True, entry[:4]  # Return bounding box coordinates
        print("No person detected.")
        return False, None

    def get_human_detection_status_with_bbox(self):
        return self.human_detected, self.human_bbox
